...
/* The below mentioned columns have the null values
country,
...
-- Total number of movies released in month wise trend
SELECT 
...
FROM
    genre;
-- There 13 unique genre list in given data set
...
-- Drama genre had produced 4285 records and the highest number of movies produced overall
...
(
SELECT
movie_id, COUNT(DISTINCT genre) AS one_genre_movie
FROM genre
GROUP BY movie_id
HAVING one_genre_movie=1
)
...
-- Action genre has the highest duration 112.88 and horror genre has the shortest duration 92.72
...
-- Thriller genre has the rank 3 and movie count of 1484
...
-- Top 3 genres are drama, comedy and action during March 2017 in the USA with more than 1,000 votes
...
SELECT 
    median_rating, COUNT(id) AS total_movie_released
FROM
    movie AS m
        INNER JOIN
    ratings AS r ON m.id = r.movie_id
...
-- Type your code below:
SELECT 
    languages, SUM(total_votes) AS total_number_votes
FROM
    movie AS M
        INNER JOIN
...
ORDER BY total_number_votes DESC; 
...
SELECT 
    n.name AS actor_name, COUNT(m.movie_id) AS movie_count
FROM
    role_mapping m
        INNER JOIN
    names n ON n.id = m.name_id
        INNER JOIN
...
COUNT(rm.movie_id) AS movie_count, Round(SUM(avg_rating*total_votes)/SUM(total_votes),2) AS actress_avg_rating,
